    ...
    return (
        <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-teal-600 to-yellow-500 flex items-center justify-center shadow-md shadow-teal-200/30">
        ...
        <div className="space-y-6">
            ...
            <Card className="p-6">
                ...
                <AnimatePresence mode="wait">
                    {cur && (
                        <motion.div key={cur.id} initial={{opacity:0,x:40}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-40}} transition={{duration:0.3}}
                            className="flex flex-col sm:flex-row items-start sm:items-center gap-6 p-5 rounded-2xl bg-gradient-to-r from-teal-600 via-yellow-50 to-white border border-teal-200">
                            <div className="flex items-center gap-4 flex-1">
                                <div className="relative">
                                    <img src={cur.image} className="w-16 h-16 rounded-2xl border-2 border-teal-300 object-cover shadow-md shadow-teal-200/50" alt="" onError={e=>{e.target.style.display='none'}} />
                                    <div className="absolute -bottom-1 -right-1 w-5 h-5 bg-gradient-to-br from-teal-600 to-yellow-500 rounded-full flex items-center justify-center border-2 border-white shadow-sm">
                                ...
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        ...
                                        <span className="text-[9px] font-black text-teal-600 bg-teal-100 border border-teal-200 px-2 py-0.5 rounded-full">{cur.symbol?.toUpperCase()}</span>
                                    </div>
                            ...
                            <div className="text-right">
                                ...
                                <p className={`text-sm font-black flex items-center justify-end gap-1 mt-0.5 ${isPos(cur.price_change_percentage_24h)?'text-sky-600':'text-teal-600'}`}>
                ...
                <div className="flex justify-center gap-1.5 mt-4">
                    {aiTokens.map((_,i)=>(
                        <button key={i} onClick={()=>setSliderId(i)} className={`h-1.5 rounded-full transition-all duration-300 ${i===sliderId%aiTokens.length?'w-6 bg-teal-500':'w-1.5 bg-teal-200'}`} />
                    ))}
                </div>
            </Card>
                ...
                <Card className="p-6">
                    ...
                    <div className="mb-4">
                        <div className="flex justify-between text-[10px] font-black mb-2">
                            ...
                            <span className="flex items-center gap-1 text-teal-600">{tokens.length-pos} ({100-posRatio}%) — Bearish<TrendingDown className="w-3 h-3"/></span>
                        </div>
                        <div className="h-5 rounded-full overflow-hidden bg-teal-100 flex border border-teal-200/50">
                    ...
                    {[{label:'Strong Up (>5%)',  n:tokens.filter(t=>Number(t.price_change_percentage_24h||0)>5).length,  color:'bg-sky-500'},
                      ...
                      {label:'Mild Down (0–5%)', n:tokens.filter(t=>{const v=Number(t.price_change_percentage_24h||0);return v<0&&v>=-5;}).length, color:'bg-teal-300'},
                      {label:'Strong Down (<-5%)',n:tokens.filter(t=>Number(t.price_change_percentage_24h||0)<-5).length, color:'bg-teal-500'},
                    ].map((b,i)=>(
                    ...
                    {heat.map((t,i)=>{
                        ...
                        return (
                            <div key={i} title={`${t.name}: ${fmtP(chg)}`} style={{background:bg}}
                                ...
                                <p className={`text-[8px] font-bold mt-0.5 ${chg>=0?'text-sky-800':'text-teal-600'}`}>{fmtP(chg)}</p>
                            </div>
                        );
                    })}
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-2">
                        {hot.slice(0,hotExpanded?20:4).map((t,i)=>(
                            <div key={i} style={{background:'rgba(254,243,199,0.45)',border:'1px solid rgba(217,119,6,0.13)'}} className="flex items-center gap-3 p-2.5 rounded-xl hover:brightness-95 transition-all">
                                <span className="text-[9px] font-black text-teal-500 w-4">#{i+1}</span>
                                <img src={t.image} className="w-8 h-8 rounded-full border border-teal-200 shadow-sm" alt="" onError={e=>e.target.style.display='none'} />
                        ...
                        <button onClick={()=>setHotExpanded(!hotExpanded)} className="w-full flex items-center justify-center gap-1.5 py-2 text-[9px] font-black text-teal-600 hover:text-teal-600 transition-colors border-t border-teal-100 mt-1 pt-3">
                        ...
                        {movers.map((t,i)=>{
                            ...
                            return (
                                <div key={i} style={{background:'rgba(255,255,255,0.75)',border:'1px solid rgba(217,119,6,0.10)'}} className="flex items-center gap-3 p-2.5 rounded-xl hover:brightness-95 transition-all">
                                    ...
                                    <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[9px] font-black border ${chg>=0?'bg-sky-50 text-sky-700 border-sky-200':'bg-teal-50 text-teal-600 border-teal-200'}`}>
                ...
                >
                    <div className="rounded-3xl p-5 border border-teal-200/70 overflow-hidden"
                        style={{
                        ...
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2">
                                <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-teal-600 to-yellow-500 flex items-center justify-center shadow-md shadow-teal-200/30">
                            ...
                            <span className="text-[8px] font-black text-teal-600 bg-teal-100 border border-teal-200 px-2 py-0.5 rounded-full">LIVE</span>
                        </div>
                        ...
                        {/* Footer note */}
                        <div className="mt-4 pt-3 border-t border-teal-100 flex items-center justify-between text-[8px] text-gray-400">
                            <span className="flex items-center gap-1"><Clock className="w-2.5 h-2.5 text-teal-500"/>Updated live</span>
...
function GrowTab() {
    ...
    const riskColors = { Low:'border-sky-200 bg-sky-50 text-sky-700', Medium:'border-teal-200 bg-teal-50 text-teal-600', High:'border-teal-200 bg-teal-50 text-teal-600' };

    return (
        <div className="space-y-10">
            {/* Staking Header */}
            <div className="text-center">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-teal-100 border border-teal-300 text-teal-600 text-[9px] font-black uppercase tracking-[0.2em] mb-4">
                    <Lock className="w-3 h-3" /> Start Earning Today
                </div>
                <h2 className="text-3xl font-black text-gray-900">Flexible <span className="bg-gradient-to-r from-teal-600 to-yellow-600 bg-clip-text text-transparent">Staking</span> Plans</h2>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5" style={{perspective:'1200px'}}>
                    {traders.map((t, i) => (
                        <TiltCard key={i}>
                            <div className="rounded-3xl p-5 border border-teal-200/60"
                                style={{
                                ...
                                <div className="grid grid-cols-2 gap-2 mb-4">
                                    <div className="rounded-xl p-3 text-center border" style={{background:'rgba(254,243,199,0.6)',borderColor:'#fde68a'}}>
                                        <p className="text-[8px] text-teal-600 uppercase font-black mb-1">90D ROI</p>
        ...
        <div className="space-y-6">
            <Card className="p-6">
                ...
                <div className="relative max-w-sm">
                    <button onClick={()=>setOpen(!open)} className="w-full flex items-center justify-between px-4 py-3 bg-teal-50 border border-teal-200 rounded-2xl text-sm font-bold text-gray-700 hover:bg-teal-100 transition-all shadow-sm">
                    ...
                    <AnimatePresence>
                        {open && (
                            <motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}}
                                className="absolute top-full left-0 right-0 mt-2 bg-white border border-teal-200 rounded-2xl shadow-xl z-30 max-h-64 overflow-y-auto">
                                {tokens.slice(0,120).map(t=>(
                                    <button key={t.id} onClick={()=>add(t)} disabled={selected.length>=10&&!selected.find(s=>s.id===t.id)}
                                        className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-teal-50 transition-all text-left disabled:opacity-30 border-b border-teal-50 last:border-0">
                                        <img src={t.image} className="w-6 h-6 rounded-full border border-teal-100" alt="" onError={e=>e.target.style.display='none'}/>
                                        ...
                                        <span className={`text-[9px] font-black ${isPos(t.price_change_percentage_24h)?'text-sky-600':'text-teal-600'}`}>{fmtP(t.price_change_percentage_24h)}</span>
                                    </button>
                                ))}
                            </motion.div>
                        )}
                        ...
                        {selected.map(t=>{
                            ...
                            return (
                                <div key={t.id} onClick={()=>setActive(isAct?null:t.id)}
                                    className={`cursor-pointer rounded-2xl p-3.5 border transition-all hover:scale-105 relative ${chg>=0?'bg-sky-50 border-sky-200':'bg-teal-50 border-teal-200'} ${isAct?'ring-2 ring-teal-500 ring-offset-1':''}`}>
                                    ...
                                    <p className={`text-[9px] font-black flex items-center gap-0.5 ${chg>=0?'text-sky-600':'text-teal-600'}`}>
                                        {chg>=0?<ArrowUpRight className="w-3 h-3"/>:<ArrowDownRight className="w-3 h-3"/>}{fmtP(chg)}
                                    </p>
                                </div>
                            );
            ...
            <AnimatePresence>
                {detail && (
                    <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} exit={{opacity:0,y:16}}>
                        <Card className="p-6 space-y-6 border-teal-300/60">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <img src={detail.image} className="w-14 h-14 rounded-2xl border-2 border-teal-300 shadow-md shadow-teal-200" alt="" onError={e=>e.target.style.display='none'}/>
                                    <div>
                                ...
                                <div className="relative w-16 h-16">
                                    ...
                                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-sm font-black text-teal-600">#{detail.market_cap_rank||'N/A'}</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                {[...Array(5)].map((_,i)=>{
                                    ...
                                    return (
                                        <div key={i} className="flex items-center gap-3 mb-2 last:mb-0">
                                            ...
                                            <div className="flex-1 h-3 rounded-full bg-teal-100 overflow-hidden flex border border-teal-100">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                                    ...
                                    <div className="flex gap-3">
                                        ...
                                        <div className="flex-1 text-center">
                                            <div className="h-20 rounded-xl bg-teal-100 flex items-end justify-center px-3 pb-2 border border-teal-200">
                                                <div className="w-full rounded-lg bg-gradient-to-t from-teal-600 to-teal-700" style={{height:`${100-longPct}%`}} />
                                            </div>
                                            <p className="text-[8px] text-teal-600 font-black mt-1.5">{100-longPct}% Bearish</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                                    ...
                                    <div className="flex flex-col items-center">
                                        ...
                                        <div className="-mt-4 text-center">
                                            <p className="text-2xl font-black text-teal-600">{fearVal}</p>
                                            <p className="text-[9px] font-black text-teal-600/80">Fear</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div className="space-y-1.5">
                                        {days10.map((d,i)=>{
                                            ...
                                            <div key={i} style={{background:'rgba(254,252,232,0.60)',border:'1px solid rgba(217,119,6,0.12)'}} className="grid grid-cols-3 rounded-xl px-3 py-1.5 text-[9px]">
                                                    ...
                                                    <span className={`text-right font-black ${Number(chg)>=0?'text-sky-600':'text-teal-600'}`}>{Number(chg)>=0?'+':''}{chg}%</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                        {mockHolders.map((h,i)=>(
                                            <div key={i} style={{background:'rgba(254,252,232,0.60)',border:'1px solid rgba(217,119,6,0.12)'}} className="grid grid-cols-3 rounded-xl px-3 py-1.5 text-[9px] items-center">
                                                <span className="text-teal-500 font-black">#{i+1}</span>
                                                <a href={`https://bscscan.com/address/${h.addr}`} target="_blank" rel="noreferrer" className="text-teal-600 hover:underline font-mono truncate">{h.addr.slice(0,9)}…</a>
    ...
    return (
        <main className="min-h-screen" style={{background:'linear-gradient(135deg,#fffdf5 0%,#fefce8 30%,#fffbeb 60%,#fef9ee 100%)'}}>
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                <div className="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full bg-teal-200/30 blur-3xl" />
                <div className="absolute top-1/2 -left-40 w-[400px] h-[400px] rounded-full bg-yellow-200/25 blur-3xl" />
                <div className="absolute bottom-0 right-1/3 w-[300px] h-[300px] rounded-full bg-teal-100/40 blur-2xl" />
            </div>
            ...
            <section className="relative pt-28 pb-24 px-4 md:px-8 max-w-7xl mx-auto">
                ...
                <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="mb-10">
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div>
                            <div className="flex items-center gap-2 mb-3">
                                <span className="flex items-center gap-1.5 text-[9px] font-black text-teal-600 bg-teal-100 border border-teal-300 rounded-full px-3 py-1 uppercase tracking-[0.2em]">
                                    <Sparkles className="w-3 h-3 text-teal-600"/> AI-Powered Intelligence
                                </span>
                            </div>
                            <h1 className="text-5xl md:text-6xl font-black leading-none text-gray-900">
                                B20 <span className="bg-gradient-to-r from-teal-600 via-yellow-500 to-teal-700 bg-clip-text text-transparent">AI</span>
                            </h1>
                        ...
                        <div className="flex flex-col items-end gap-2">
                            <button onClick={loadData} disabled={loading} className="flex items-center gap-2 px-4 py-2 bg-white border border-teal-200 rounded-full text-xs font-bold text-gray-600 hover:bg-teal-50 transition-all shadow-sm disabled:opacity-50">
                                <RefreshCw className={`w-3.5 h-3.5 text-teal-600 ${loading?'animate-spin':''}`}/> Refresh
                            </button>
                            {lastUp && <p className="text-[9px] text-gray-400 flex items-center gap-1"><Clock className="w-2.5 h-2.5 text-teal-500"/>Updated {lastUp.toLocaleTimeString()}</p>}
                        </div>
                    </div>
                </motion.div>

                {/* Tab bar */}
                <div className="flex bg-white/70 border border-teal-200 p-1.5 rounded-2xl mb-8 max-w-md shadow-sm backdrop-blur-sm">
                    {TABS.map(t=>{
                        const Icon=t.icon;
                        return (
                            <button key={t.id} onClick={()=>setTab(t.id)}
                                className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.1em] transition-all ${tab===t.id?'bg-gradient-to-r from-teal-600 to-yellow-500 text-white shadow-lg shadow-teal-200/40':'text-gray-400 hover:text-teal-600'}`}>
