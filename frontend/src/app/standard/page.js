    ...
    return (
        <main className="min-h-screen bg-gray-50/70 p-pattern selection:bg-teal-500 selection:text-white pb-32">
            ...
            <div className="pt-10 pb-24 px-4 md:px-8 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-10 relative z-10">
                ...
                <div className="absolute top-0 left-1/4 w-[600px] h-[600px] bg-teal-500/5 rounded-full blur-[120px] -z-10 animate-pulse" />
                <div className="absolute bottom-0 right-1/4 w-[600px] h-[600px] bg-teal-500/5 rounded-full blur-[120px] -z-10" />

                {/* LEFT: SIDEBAR */}
                <div className="lg:col-span-4 space-y-8">
                    <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }}
                        className="p-10 rounded-[3rem] bg-white border border-gray-100 shadow-xl relative overflow-hidden group">
                        <div className="absolute -top-12 -right-12 w-32 h-32 bg-teal-500/10 rounded-full blur-3xl group-hover:bg-teal-500/20 transition-all duration-700" />
                        ...
                        <div className="pt-6 border-t border-gray-50 flex items-center gap-2 text-teal-600">
                    ...
                    <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }}
                        ...
                        <h4 className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-8 flex items-center gap-2">
                             <Settings className="w-4 h-4 text-teal-600" /> Protocol Config
                        </h4>
                        <div className="space-y-6">
                            {[
                                ...
                                { label: 'Network', value: 'BNB Smart Chain', icon: <Globe className="w-4 h-4" />, color: 'text-sky-500' }
                            ].map((p, i) => (
                ...
                <div className="lg:col-span-8">
                    <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="p-12 rounded-[3.5rem] bg-white border border-gray-100 shadow-2xl relative overflow-hidden backdrop-blur-3xl">
                        ...
                        <AnimatePresence>
                            {(status === 'uploading' || status === 'success' || status === 'error') && (
                                ...
                                >
                                    {status === 'uploading' && (
                                        <>
                                            <div className="w-24 h-24 border-4 border-teal-500/10 border-t-blue-500 rounded-full animate-spin mb-10" />
                                            <h3 className="text-4xl font-black text-gray-900 mb-4 uppercase tracking-tighter">Deploying Legacy...</h3>
                                            <p className="text-teal-600 font-black uppercase text-[10px] tracking-widest">{error}</p>
                                        </>
                                    )}
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-16 border-b border-gray-50 pb-12">
                            <div>
                                <h1 className="text-5xl font-black text-gray-900 tracking-tighter mb-2">Standard <span className="text-teal-600">Asset</span></h1>
                                <div className="flex items-center gap-4">
                                    <span className="px-4 py-1.5 bg-teal-500 text-white text-[10px] font-black uppercase tracking-[0.2em] rounded-full shadow-lg shadow-teal-200/20">Nexus Proxy</span>
                                    <span className="px-4 py-1.5 bg-teal-500/10 text-teal-600 text-[10px] font-black uppercase tracking-[0.2em] rounded-full border border-teal-500/20">Legacy Mode</span>
                                </div>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-12">
                            <div className="flex flex-col md:flex-row items-center gap-10">
                                <div className="relative group">
                                    <div className="w-44 h-44 rounded-[3.5rem] bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden transition-all duration-500 group-hover:scale-105 group-hover:border-teal-500/30 shadow-inner">
                                    ...
                                    <div className="absolute -bottom-2 -right-2 bg-teal-500 text-white p-3 rounded-2xl shadow-xl"><Globe className="w-5 h-5" /></div>
                                </div>
                                <div className="flex-1 space-y-6 w-full">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Legacy Name</label>
                                            <input type="text" placeholder="Institutional Token" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 font-bold text-gray-900 outline-none focus:bg-white focus:border-teal-500/30 transition-all shadow-sm" />
                                        </div>
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Asset Symbol</label>
                                            <input type="text" placeholder="ITK" value={formData.symbol} onChange={(e) => setFormData({...formData, symbol: e.target.value.toUpperCase()})} className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 font-black text-gray-900 outline-none focus:bg-white focus:border-teal-500/30 transition-all shadow-sm" />
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Decimals</label>
                                            <input type="number" value={formData.decimals} onChange={(e) => setFormData({...formData, decimals: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 font-bold text-gray-900 outline-none focus:bg-white focus:border-teal-500/30 transition-all shadow-sm" />
                                        </div>
                                        <div className="space-y-3">
                                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-2">Fixed Supply</label>
                                            <input type="number" value={formData.supply} onChange={(e) => setFormData({...formData, supply: e.target.value})} className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 font-bold text-gray-900 outline-none focus:bg-white focus:border-teal-500/30 transition-all shadow-sm" />
                                        </div>
                                    </div>
                                </div>
                            </div>
                                                            ...
                                                            {mimicData.similarTokens.slice(0, 2).map((t, i) => (
                                                                <div key={i} className="bg-white/60 p-3 rounded-2xl border border-black/5">
                                                                    ...
                                                                    <div className="flex items-center justify-between bg-white/50 px-2 py-1.5 rounded-lg border border-black/5 group">
                                                                         <code className="text-[9px] font-mono text-teal-600 truncate">{t.contractAddress ? `${t.contractAddress.slice(0,6)}...${t.contractAddress.slice(-4)}` : 'UNKNOWN'}</code>
                                                                         <button type="button" onClick={(e) => { e.preventDefault(); navigator.clipboard.writeText(t.contractAddress); alert('Copied!'); }} className="opacity-0 group-hover:opacity-100 transition-opacity">
                                                                             <Copy className="w-3 h-3 text-gray-400 hover:text-teal-600" />
                                                                         </button>
                                                                    </div>
                                                                </div>
                                                            ))}
