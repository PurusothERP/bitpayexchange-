        ...
        <div>
            <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
                ...
                <div className="relative w-full md:w-96 group">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-rose-500 transition-colors" />
                    <input
                        type="text"
                        placeholder="Search tokens..."
                        value={search}
                        onChange={(e) => setSearch(e.target.value)}
                        className="w-full bg-black/5 border border-black/10 rounded-2xl py-4 pl-12 pr-4 focus:outline-none focus:border-rose-500/50 transition-all text-sm font-medium"
                    />
                </div>
            </div>
