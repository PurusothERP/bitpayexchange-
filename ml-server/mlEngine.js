...
async function detectMimicToken(name, symbol) {
  ...
  for (const token of tokens) {
    ...
    if (nameSim >= 0.75 || symbolSim >= 0.8) {
      results.push({
        token,
        nameSim:   Math.round(nameSim   * 100),
        symbolSim: Math.round(symbolSim * 100),
        type:  'SIMILAR',
        severity: nameSim >= 0.92 || symbolSim >= 0.92 ? 'HIGH' : 'MEDIUM'
      });
    }
  }
  ...
  if (topMatches.length > 0) {
    ...
    if (top.severity === 'CRITICAL') {
      riskLevel    = 'CRITICAL';
      alertMessage = `⚠️ CRITICAL: "${name}" (${symbol}) is an exact match for ${top.token.name} (Rank #${top.token.rank}).`;
      duplicateReport = `The token "${name}" is already registered on major indexes. Deploying this may result in your token being flagged as a phishing attempt on MetaTask and BscScan.`;
    } else if (top.severity === 'HIGH' || patternFlag) {
      riskLevel    = 'HIGH';
      alertMessage = `🔴 HIGH RISK: Close similarity detected with ${top.token.name}.`;
      duplicateReport = `Our Neural Scanners found a ${top.nameSim}% name match with an existing top-ranked token. This is statistically likely to be flagged as a 'copycat' contract.`;
    } else {
      riskLevel    = 'MEDIUM';
      alertMessage = `🟡 WARNING: Minor similarity with ${top.token.name}.`;
      duplicateReport = `While not an exact mimic, your token shares naming conventions with existing assets. Consider adding a unique prefix or suffix.`;
    }
  }
...
async function scoreTokenIntelligence(name, symbol) {
  ...
  // MIMIC CHECK (Integrated)
  const mimic = await detectMimicToken(name, symbol);
  ...
  const memorability = Math.min(100, Math.max(10,
    (wordLen <= 5  ? 90 : wordLen <= 8 ? 75 : wordLen <= 12 ? 60 : 40) +
    (syllables >= 1 && syllables <= 3 ? 10 : 0) +
    (hasNumbers ? -15 : 0)
  ));
  ...
  // MARKET APPEAL: AI and Neural keywords score significantly higher
  const bullKeywords  = ['ai','defi','web3','neural','brain','gpt','agent','autonomous','omni','nexus','hyper','quantum','nova','apex','flux','zen'];
  const marketAppeal = Math.min(100, Math.max(10, 60 + (bullKeywords.some(k => n.includes(k)) ? 30 : 0)));

  // RISK LEVEL: higher = more risky
  const topTokenSim = tokens.slice(0, 150).reduce((max, t) =>
    Math.max(max, similarityScore(n, t.name), similarityScore(s, t.symbol)), 0);
  
  const riskLevel = Math.min(100, Math.max(5,
    Math.round(topTokenSim * 85) +
    (mimic.riskLevel === 'CRITICAL' ? 60 : mimic.riskLevel === 'HIGH' ? 40 : 0)
  ));

  const overall = Math.round((memorability + uniqueness + marketAppeal + (100 - riskLevel)) / 4);

  let verdict = '';
  if (overall >= 80) verdict = `Elite Tier — Perfect neural symmetry.`;
  else if (overall >= 60) verdict = `Strong Tier — Solid market-ready candidate.`;
  else verdict = `Warning Tier — Collision risk detected.`;

  return { 
    memorability, uniqueness, marketAppeal, riskLevel, overall, verdict,
    duplicateReport: mimic.duplicateReport,
    similarTokens: mimic.similarTokens,
    riskRating: mimic.riskLevel
  };
}
...
Return this exact JSON structure:
{
  ...
  try {
    ...
    // FALLBACK: Return a high-quality mock response if API fails
    // This ensures the UI doesn't break and users can see the potential
    const mock = {
      tokenName:   `${prompt.split(' ').slice(-1)[0]}AI`,
      tokenSymbol: prompt.slice(0, 4).toUpperCase(),
      description: `A next-generation digital asset conceived by the antigraVITY AI Engine. Inspired by your prompt: "${prompt}", this token aims to disrupt its category with decentralized innovation.`,
      tagline:      "Engineered by AI, Powered by Community.",
      category:     "ai",
      suggestedChain: "bsc",
      chainReason:  "BSC offers the best balance of low fees and high liquidity for new AI-branded assets.",
      initialSupply: 1000000000,
      decimals:     18,
      features:     ["AI-Burn", "Community DAO", "Neural Yield"],
      targetAudience: "Early adopters and believers in the intersection of AI and Blockchain.",
      launchStrategy: "Launch on PancakeSwap, initiate community airdrops, and lock liquidity for 365 days.",
      palette:      ["#8B5CF6", "#C4B5FD", "#EDE9FE", "#2E1065"],
      logoEmoji:    "🤖",
      estimatedGasCost: "$25.00",
      riskWarnings:  ["Neural Engine running in Safe Mode due to API rate limits."]
    };
    const mimicCheck = await detectMimicToken(mock.tokenName, mock.tokenSymbol);
    mock.mimicAlert = mimicCheck.riskLevel !== 'SAFE' ? mimicCheck.alertMessage : null;
